...
pub fn App() -> impl IntoView {
    let UseWebSocketReturn { message, send, .. } = use_websocket("ws://localhost:3030");
                ...
                if evt.event_type == CLIENT_LIST {
                    if let Ok(cl) = serde_json::from_value::<ClientListEvent>(evt.data) {
                        set_clients.update(|c| {
                            *c = cl.clients.into_iter().map(|c| c.name).collect::<Vec<String>>()
                        });
                    }
    ...
    view! {
        <div class="app">
            <div class="container">
                ...
                <Connect send={send} set_name={set_name} /> // send is use_websocket helper and is a util function
    ...
    F: Fn(&str) + Clone + 'static,
{
    ...
    let submit_handler = move |ev: SubmitEvent| {
        ...
        send(&format!(r#"{{ "event_type": "INIT", "data": {{ "name": "{}" }} }}"#, name,));
